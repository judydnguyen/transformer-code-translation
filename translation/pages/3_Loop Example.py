...
st.markdown('<div class="custom-text">The final code snippet we will translate to C# from Java is a loop that prints numbers to the console. In particular, this code snippet tests the model\'s ability to pair functions from each languages standard libraries.</div>', unsafe_allow_html=True)
st.markdown('')
...
if st.button("Translate"):
    ...
    expected_code_output = code_editor("""public static void PrintNumbers(int n) { 
        for (int i = 0; i < n; i++) { 
            Console.WriteLine(i); 
        } 
    }""", lang="csharp")
